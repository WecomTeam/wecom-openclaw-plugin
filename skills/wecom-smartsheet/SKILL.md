...
| `851003` | `no authority` | 无文档权限。在编辑场景，通常是因为机器人不允许编辑由企业成员创建的文档。在**智能表格写入场景**下，若该报错发生在 `smartsheet_add_records` / `smartsheet_update_records`，可引导用户走 Webhook 兜底方案，详见 [webhook-fallback.md](references/webhook-fallback.md)。注意 Webhook 只能更新通过 Webhook 写入的记录，人工创建的记录无法更新 | — |
...
| 删除记录 | 见下方「删除记录」 |
| Webhook 兜底写入 | [references/webhook-fallback.md](references/webhook-fallback.md) |
...
- 通过 docid：使用 `wecom_mcp` tool 调用 `wecom_mcp call doc smartsheet_get_sheet '{"docid": "DOCID"}'`
- 通过 URL：使用 `wecom_mcp` tool 调用 `wecom_mcp call doc smartsheet_get_sheet '{"url": "https://doc.weixin.qq.com/smartsheet/xxx"}'`
...
**只能改名，不能改类型**（`field_type` 必须传原始类型）。`field_title` 不能更新为原值。
...
- 通过 docid：使用 `wecom_mcp` tool 调用 `wecom_mcp call doc smartsheet_get_records '{"docid": "DOCID", "sheet_id": "SHEETID"}'`
...
### smartsheet_add_records — 添加记录（不带图片或文件）

添加一行或多行记录，单次建议 500 行内。

**调用前**必须先了解目标表的字段类型（通过 `smartsheet_get_fields`），并查看[单元格值格式参考](references/smartsheet-cell-value-formats.md)，了解做法后再实施。对于单选/多选（Option）字段，需注意匹配已有选项的 `id`。
...
### smartsheet_add_records_auto_file — 添加记录（带图片或文件）

添加一行或多行记录，单次建议 500 行内。与 `smartsheet_add_records` 不同之处在于，可支持本地路径传入图片、文件。对于需要添加带图片或文件的记录，请使用此接口。传入后台后，后台将自动存储并转换为 `image_url`。

使用 `wecom_mcp` tool 调用 `wecom_mcp call doc smartsheet_add_records_auto_file '{"docid":"DOCID","sheet_id":"SHEETID","records":[{"values":{"图片":[{"image_path":"/path/to/image.jpg"}],"文件":[{"file_path":"/path/to/file.txt"}]}}]}'`
...
使用 `wecom_mcp` tool 调用 `wecom_mcp call doc smartsheet_update_records '{"docid": "DOCID", "sheet_id": "SHEETID", "key_type": "CELL_VALUE_KEY_TYPE_FIELD_TITLE", "records": [{"record_id": "RECORDID", "values": {"任务名称": [{"type": "text", "text": "更新后的内容"}]}}]}'`
...
使用 `wecom_mcp` tool 调用 `wecom_mcp call doc smartsheet_update_records_auto_file '{"docid": "DOCID", "sheet_id": "SHEETID", "key_type": "CELL_VALUE_KEY_TYPE_FIELD_TITLE", "records": [{"record_id": "RECORDID", "values": {"图片":[{"image_path":"/path/to/image.jpg"}],"文件":[{"file_path":"/path/to/file.txt"}]}}]}'`
...
1. **新建智能表格** → 使用 `wecom_mcp` tool 调用 `wecom_mcp call doc create_doc '{"doc_type": 10, "doc_name": "项目任务表"}'`，保存返回的 `docid`
...
1. **了解表结构** → 使用 `wecom_mcp` tool 调用 `wecom_mcp call doc smartsheet_get_sheet '{"docid": "DOCID"}'` → 使用 `wecom_mcp` tool 调用 `wecom_mcp call doc smartsheet_get_fields '{"docid": "DOCID", "sheet_id": "SHEETID"}'`
...
| 依赖技能 | 典型协作场景 | 数据流向 |
|---|---|---|
| `wecom-contact` | 成员（USER）类型字段需填 `user_id`，不能直接用姓名 | `get_userlist` 按姓名查到 userid → 本 skill 写入 |
| `wecom-msg` | 用户要求把智能表格链接发给某人/某群 | 本 skill 新建后返回 `url` → `wecom-msg` 发送链接 |

> **注意**：成员（USER）类型字段需要填写 `user_id`，不能直接使用姓名。必须先通过 `wecom-contact` 技能的 `get_userlist` 接口按姓名查找到对应的 `userid` 后再使用。
