...
wecom-todo 提供企业微信待办事项的管理能力，包含以下功能：
...
使用 `wecom_mcp` tool 调用 `wecom_mcp call todo search_todo_userid '<json格式的入参>'`
...
使用 `wecom_mcp` tool 调用 `wecom_mcp call todo search_todo_userid '{"keyword": "张三"}'`
...
| `remind_type_list` | uint32[] | 否  | 提醒方式，相对 `end_time` 计算，可传入多个，取值见注意事项第 4 条                       |

#### 调用示例

使用 `wecom_mcp` tool 调用 `wecom_mcp call todo create_todo '{"content": "<待办的内容>", "follower_list": {"followers": [{"follower_id": "FOLLOWER_ID", "follower_status": 1}]}, "end_time": "2025-06-01 09:00:00", "remind_type_list": [3, 7]}'`
...
| `remind_type_list` | uint32[] | 否 | 新的提醒方式，相对 `end_time` 计算，可传入多个                                |

#### 调用示例

使用 `wecom_mcp` tool 调用 `wecom_mcp call todo update_todo '{"todo_id": "TODO_ID", "content": "<待办的内容>", "end_time": "2025-07-01 09:00:00", "remind_type_list": [1]}'`
...
使用 `wecom_mcp` tool 调用 `wecom_mcp call todo change_todo_user_status '{"todo_id": "TODO_ID", "follower_id": "FOLLOWER_ID", "user_status": 2}'`
...
使用 `wecom_mcp` tool 调用 `wecom_mcp call todo get_todo_list '{"follower_id": "FOLLOWER_ID", "create_begin_time": "2025-06-01 00:00:00", "create_end_time": "2025-06-30 23:59:59", "todo_status": 1, "limit": 20}'`
